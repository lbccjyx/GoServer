...
默认可执行文件名为 **`DedicatedServer.exe`**。若要改名，请改 `matchmaker/dedicated.go` 里 `resolveDedicatedServerExe` 中的常量 `defaultName`。
...
- `{"type":4,"reason":"..."}`：房间解散
- `{"type":5,"online":<n>,"matching":<n>,"in_game":<n>,"rooms":[...]}`：状态广播  
  `rooms` 每项结构：`{"id":"room-wait-1","name":"匹配房间","player_count":2,"max_players":4,"wait_seconds":8,"player_names":["Alice","Bob"]}`
- `{"type":7,"name":"昵称"}`：写入当前连接 IP 对应昵称（服务端内存 HashMap + 二进制持久化）
- `{"type":7}`：查询当前连接 IP 对应昵称；返回 `{"type":7,"ok":true,"name":"..."}`（可能为空）
- `{"type":"error","reason":"..."}`：错误消息

说明：
- `type` 为数值消息时，按 `0/1/2/3/4/5/6/7` 固定语义处理；
- `type=="error"` 是唯一字符串类型；
- 未在上述约定中的字段/事件名（如 `event`、`payload`、`queue_size` 等）不作为正式协议输入。

### 昵称持久化（新增）

- 昵称映射（`IP Hash -> 昵称`）会保存到二进制文件：`name_store.bin`（位于 `matchserver.exe` 同目录）。
- 匹配服启动时自动读取该文件；优雅关服（SIGINT/SIGTERM，触发 `Shutdown`）时自动写回。
- 序列化格式使用 Go `encoding/gob`（二进制），不是 JSON/TXT。

### 房间解散语义（当前实现）

- **等待房（未开局）**：若有人退出导致仅剩 1 人，房间立即解散，剩余玩家收到 `type=4`，`reason="人数不足，房间已关闭"`。
- **已开局房间**：若玩家离开/断线后，匹配服视角仅剩 1 人在线，房间会被动解散，剩余玩家同样收到 `type=4`（`人数不足，房间已关闭`）。
- 房主主动解散时，`reason="房主解散房间"`。
...
**Q：房间解散后端口会怎样？**  
A：匹配服会结束专服进程，并将端口放回内部池子供下次匹配复用（后续可能复用旧端口）。
...
**Q：大厅里如何展示“匹配中的房间 + 房内人名 + 倒计时”？**  
A：读取 `type=5` 的 `rooms` 字段：每个 `room` 的 `player_names` 与 `wait_seconds` 就是展示源数据，不需要客户端本地兜底推断。
